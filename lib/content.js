...
// the admin panel writes to it. Reads are mtime-cached (re-parsed only when the
// file actually changes) with an in-memory last-good fallback so a bad write can
// never take the site down.

import { readFileSync, writeFileSync, renameSync, mkdirSync, existsSync, copyFileSync, statSync } from "fs";
...
export function saveContent(obj) {
  ...
  lastGood = obj;
  cached = obj;
  try { cachedMtime = statSync(CONTENT_PATH).mtimeMs; } catch { cachedMtime = 0; }
  return obj;
}
